...
const ListUsers = () => {
  ...
  const loadData = () => http.get('/cms/users')
        ...
        <div className="h-full flex flex-col min-[622px]:flex-row md:flex-row p-4 items-center gap-10">
          <div className="p-3 min-w-full">
            <table className=" table-auto min-w-full">
              ...
              <tbody>
                {users.map(user => (
                  <tr key={user._id} className=" even:bg-cyan-400 odd:bg-white">
                  <td className="p-3 text-sm text-gray-800">{user.name}</td>
                  <td className="p-3 text-sm text-gray-800">{user.email}</td>
                  <td className="p-3 text-sm text-gray-800">{user.status.toString()}</td>
                  <td className="p-3 text-sm text-gray-800">{user.userType}</td>
                  <td className="p-3 text-sm text-gray-800">{formatDate(user.updatedAt)}</td>
                  <td className="p-2 text-sm text-gray-800">
                    <Link className="inline-flex items-center px-4 py-2 hover:bg-lime-200 text-gray-800 text-sm font-medium rounded-md" to={`/cms/users/${user._id}/edit`}>
                  ...
                  <td className="p-2 text-sm text-gray-800">
                  <button onClick={() => handleDelete(user._id)} className="inline-flex items-center px-4 py-2 hover:bg-red-400 hover:text-gray-800 text-sm font-medium rounded-md">
